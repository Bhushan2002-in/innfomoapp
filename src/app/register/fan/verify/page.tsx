...
export default function FanVerifyPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    router.push("/register/fan/email");
  };
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-md flex-col overflow-hidden px-6 py-6">
        <div className="flex items-center justify-between pt-4 pb-2">
          <button
            type="button"
            onClick={() => router.back()}
            className="group flex h-12 w-12 items-center justify-center rounded-full bg-surface-light text-gray-900 shadow-sm transition-all hover:border-primary/20 active:scale-95 dark:bg-surface-dark dark:text-white"
            aria-label="Go back"
          >
            <ArrowLeftIcon className="text-2xl transition-transform group-hover:-translate-x-0.5" />
          </button>
        ...
        <div className="mt-auto pt-6 pb-4">
          <button
            type="submit"
            form="fan-verify-form"
            className="group relative flex w-full items-center justify-center rounded-2xl bg-primary py-4 text-lg font-bold text-white shadow-[0_10px_30px_-10px_rgba(108,43,238,0.6)] transition-all duration-300 hover:bg-[#5820c6] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 active:scale-[0.98]"
          >
            <span>Continue</span>
